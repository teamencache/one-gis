let vertexStr = `
    ...
    void main(){
        ...
        gl_PointSize = 4.0;
        if(a_Offset>0.0){
            v_TexCoord = vec2(1.0, a_Length/u_TotalLength);
        }else{
            v_TexCoord = vec2(0.0, a_Length/u_TotalLength);
        }
...
let fragmentStr = `
    ...
    void main(){
        float texCoord_t = fract(v_TexCoord.t + u_Time);
        vec4 texColor = texture2D(u_Sampler2D, vec2(v_TexCoord.s, texCoord_t));
        /* if(abs(v_TexCoord.s-0.5)>0.1){
...
function initOffsetLineVertex() {
    ...
    while (vertexIndex < vertexLength) {
        ...
        if (!vertexIndex) {
            ...
            createOffsetData(currentVertex, currentNormal, 0.0, vertexIndex);
            vertexIndex++;
            continue;
        }
        //最后一个点
        if (vertexIndex == vertexLength - 1) {
            createOffsetData(currentVertex, lastInfo.normal, lastInfo.length, vertexIndex);
            break
        }
...
function createOffsetData(position, direct, segmentLength, vertexIndex) {
    ...
    offsetVertex.push(variable.u_TotalLength.value);//length
    // 反
